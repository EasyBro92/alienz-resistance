...
| Huéspedes animados (en `assets.js`) | `ALIEN_ANIMADOS`: Portador, Encostrado (`-andar.glb`) y Saltador (`-correr.glb`) llevan esqueleto de Meshy (`herramientas/meshy-rig.mjs --nombre alien-x`, 5 créditos, lee el .glb de la web publicada; los modelos en postura rara fallan con "Pose estimation failed", como el Corredor). `alienAnimado` recorre el clip según la fase de `zombie.js` (`ciclos` por vuelta) y hace el zarpazo por código girando tronco y brazos sobre el eje lateral del mundo (positivo adelanta lo que cuelga). El Saltador va con `sinZarpazo` (Isidro lo eligió así): su esqueleto de Meshy estira las púas en láminas si se giran los brazos. Quedan 5 créditos guardados para cuando recargue.

**Pendiente al recargar Meshy (encargo de Isidro, 15/09/2026):** rehacer el Escarbador de pie, sin tierra bajo los pies y con brazos separados, y ponerle esqueleto con `meshy-rig.mjs` (unos 20 créditos en total); verificarlo todo antes de publicar. Su modelo actual lleva la tierra fundida con los pies en una sola isla de malla y agachado, así que el esqueleto por código lo deja hecho un amasijo. Rehacer también el Corredor de pie para que Meshy le acepte el esqueleto. Prompt previsto: `node herramientas/meshy.mjs --nombre alien-escarbador --criatura --refinar --modelo meshy-5 --polys 6000 --prompt "alien zombie hybrid digger creature, infected former human, standing upright in neutral pose, arms held slightly away from the body, dark brown leathery skin with black chitin plates, two large curved horns on head, one arm ends in a metal drill, glowing green eyes and veins, feet flat, no ground, no base, no rocks, full body"` Punto de guardado del esqueleto por código: etiqueta `guardado-esqueleto-codigo`. Todos se repintan con `PALETA_ALIEN` (`aclararMaterial`: rampa coraza→piel ecualizada, sin metal, algo de luz propia) |
